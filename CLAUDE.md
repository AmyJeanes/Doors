...
- **The aim term deliberately fades out close to a doorway.** `facing` flips sign the instant the *eye* crosses the doorway plane, which is a step before the body is teleported through — leaving a band where you are geometrically behind the opening but still in the space in front of it. A raw dot product costs an audible 6 dB there, so it is blended back toward head-on across the near field (half a doorway). Don't "simplify" it back.
