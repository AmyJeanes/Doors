...
Six things worth knowing before you change any of it:

- **A sound can be several doorways away, and `resolve` walks all of them** — a box parked inside another box puts two between its sounds and the world. `doors_sound_resolution` carries both views: `hops` (the first `hop_count` of them) describes each doorway on the path, and the scalar fields are the totals across it. The debug graph draws one continuous curve by walking `hops`, so a new per-doorway term has to land on the hop as well as in the total or the graph and the audio disagree.
- **The aim term deliberately fades out close to a doorway.** `facing` is measured from the *clamped* mouth point, so a listener within the doorway's rectangle leaves only the perpendicular component — the direction is then exactly ±normal, and its sign turns purely on which side of the plane they stand. A door frame has depth, so standing in one (outside, a unit or two past the plane, and still, so nothing triggers a crossing) reads as round the back and costs an audible 6 dB. This is **not** a misplaced threshold — measured in-game, the doorway plane and the `linked_portal_door` agree to 0.03u. Blending back toward head-on across the near field is the fix; don't "simplify" it away.
